...
fn remember_and_reopen_persists() {
    ...
    let link = {
        ...
        storage.remember("facts", "Paris is the capital of France", Some(("name", "paris")), false, None).unwrap()
    };
    let storage2 = dir.open();
    let results = storage2.recall("facts", "paris capital", 5).unwrap();
...
fn remember_with_key_upserts() {
    ...
    assert_eq!(l1, l2, "same key should return same link on update");
}
...
fn remember_without_key_creates_unique() {
    ...
    assert_ne!(l1, l2, "no key means new document each time");
}
...
fn recall_keyword_finds_relevant() {
    ...
    assert!(!results.is_empty(), "keyword recall should find matches");
}
...
fn forget_removes_document() {
    ...
    let mut storage = dir.open();
    let link = storage.remember("facts", "temporary", None, false, None).unwrap();
    storage.forget(&link).unwrap();
    // Re-remember with same content — should get new link since old was deleted
    let new_link = storage.remember("facts", "temporary", None, false, None).unwrap();
    assert_ne!(link, new_link, "forgotten document gets new link on re-insert");
}
...
fn relate_and_neighbors() {
    ...
    assert_eq!(neighbors.len(), 1, "Alice should have one neighbor");
    assert_eq!(neighbors[0].1, "knows", "edge type should be 'knows'");
}
...
fn path_finds_shortest_route() {
    ...
    assert_eq!(path.len(), 3, "path A→B→C should have 3 nodes");
}
...
fn traverse_respects_depth() {
    ...
    storage.relate(&a, &b, "links", None).unwrap();
    storage.relate(&b, &c, "links", None).unwrap();
    assert_eq!(storage.traverse(&a, 1, None).unwrap().len(), 1, "depth 1 = one neighbor");
    assert_eq!(storage.traverse(&a, 2, None).unwrap().len(), 2, "depth 2 = two nodes");
}

#[test]
fn neighbors_with_type_filter() {
    let dir = TestDir::new("type_filter");
    let mut storage = dir.open();
    let a = storage.remember("nodes", "A", Some(("name", "a")), false, None).unwrap();
    let b = storage.remember("nodes", "B", Some(("name", "b")), false, None).unwrap();
    storage.relate(&a, &b, "knows", None).unwrap();
    storage.relate(&a, &b, "imports", None).unwrap();
    assert_eq!(storage.neighbors(&a, Some("knows")).unwrap().len(), 1, "filter by 'knows'");
}
...
fn unwrap_config_include_types() {
    ...
    assert_eq!(results.len(), 1, "only 'knows' edges followed");
}
...
fn unwrap_config_exclude_types() {
    ...
    let (results, _) = storage.traverse_with_config(&a, &config).unwrap();
    assert_eq!(results.len(), 1, "audit_log excluded, only knows remains");
}
...
fn list_collections_by_prefix() {
    ...
    let proj = storage.list_collections("project:test:");
    assert!(proj.len() >= 2, "should find project collections");
    assert!(!storage.list_collections("user:").is_empty(), "should find user collections");
}
...
fn collection_isolation_works() {
    ...
    let mut storage = dir.open();
    storage.remember("project:a:files", "file in project A", None, false, None).unwrap();
    storage.remember("project:b:files", "file in project B", None, false, None).unwrap();
    let a = storage.recall("project:a:files", "file", 5).unwrap();
    let b = storage.recall("project:b:files", "file", 5).unwrap();
    assert!(!a.is_empty(), "project A has data");
    assert!(!b.is_empty(), "project B has data");
}
